...
tbl_lifetables <- readr::read_csv(
  here::here('data/lifetables.csv.gz'),
  show_col_types = FALSE
)
...
tbl_lifetables |> 
  ...
  ggplot(aes(x = year, y = lx/10^5, color = "red")) +
  # Add a step layer
  geom_step() +
  # Add labels (title, subtitle, x, y, caption)
  labs(
    title = "Probability of Being Alive in 2022",
    subtitle = "Based on Year of Birth",
    x = "year of birth",
    y = "probability of being alive",
    caption = "Source: SSA Acturial life tables"
  ) +
...
tbl_lifetables_extended_2022 |> 
  ...
  # Add labels (title, subtitle, x, y, caption)
   labs(
    title = "Extended Probability of Being Alive in 2022",
    subtitle = "Based on Year of Birth and Age",
    x = "year of birth",
    y = "probability of being alive",
    caption = "Source: SSA Acturial life tables"
   )
...
plot_estimated_age <- function(tbl, my_name, my_sex) {
  tbl |> 
    ...
    geom_col(aes(y = nb_alive), color = 'grey') +
    # Add a line layer with y = nb_births, and appropriate colors
    geom_line(aes(y = nb_alive), color = 'lightgreen') +
    ...
    labs(
      ...
      subtitle = "Based on Name, Age and Probability of being Alive",
      x = "year of birth",
      y = "# of people alive"
    ) +
...
tbl_names_extended_age |> 
  ...
  labs(
    ...
    subtitle = expression(italic("Based on median age and probablity of being alive")),
    x = "median age",
    y = "name"
  ) +
  ...
  theme(
    ...
    panel.grid.minor.x = element_line(linetype = "dashed", linewidth = 0.3)
  ) 
```
